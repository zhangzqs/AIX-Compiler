...
public @interface ActionElement {
	...
	/**
	 * The fully qualified name of the action. For standard actions defined in the
	 * {@link android.content.Intent} class, prepend "android.intent.action" to the
	 * "string" in each ACTION_string constant. For example, to specify ACTION_MAIN
	 * the fully qualified name would be "android.intent.action.MAIN". Custom
	 * defined actions are conventionally prepended with the package name of their
	 * containing class, e.g. "com.example.project.ACTION". The name attribute is
	 * required in any @ActionElement annotation and hence has no default value.
	 *
