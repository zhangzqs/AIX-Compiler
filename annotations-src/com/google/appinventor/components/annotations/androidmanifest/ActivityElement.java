...
/**
 * Annotation to describe an <activity> element required by a component so that
 * it can be added to AndroidManifest.xml. <activity> element attributes that
 * are not set explicitly default to "" or {} and are ignored when the element
 * is created in the manifest.
 *
...
public @interface ActivityElement {
	 ...
	 * Inventor component. The name attribute is required in any @ActivityElement
	 ...
	 * activity in it's context stream, which resides in another process. The
	 ...
	 * "true" if it can move, and "false" if it must remain with the task where it
	 * started.
	 *
	 ...
	 * activity that started it and it stays there for its entire lifetime. You can
	 * use this attribute to force it to be re-parented to the task it has an
	 * affinity for when its current task is no longer displayed. Typically, it's
	 * used to cause the activities of an application to move to the main task
	 * associated with that application.
	 *
	 * For example, if an e-mail message contains a link to a web page, clicking the
	 * link brings up an activity that can display the page. That activity is
	 ...
	 * attribute. The affinity of a task is determined by reading the affinity of
	 * its root activity. Therefore, by definition, a root activity is always in a
	 * task with the same affinity. Since activities with "singleTask" or
	 * "singleInstance" launch modes can only be at the root of a task, re-parenting
	 * is limited to the "standard" and "singleTop" modes. (See also the
	 * {@link #launchMode()} attribute.)
	 *
	 ...
	 * maintained by the system — "true" if it will be, and "false" if the system is
	 * allowed to reset the task to its initial state in certain situations. The
	 * default value is "false". This attribute is meaningful only for the root
	 * activity of a task; it's ignored for all other activities. Normally, the
	 * system clears a task (removes all activities from the stack above the root
	 * activity) in certain situations when the user re-selects that task from the
	 * home screen. Typically, this is done if the user hasn't visited the task for
	 * a certain amount of time, such as 30 minutes.
	 *
	 * However, when this attribute is "true", users will always return to the task
	 * in its last state, regardless of how they get there. This is useful, for
	 * example, in an application like the web browser where there is a lot of state
	 ...
	 * The default value is "false". This attribute is meaningful only for
	 * activities that start a new task (the root activity); it's ignored for all
	 ...
	 * situations (see the {@link #alwaysRetainTaskState()} attribute), but not
	 * always.
	 *
	 * Suppose, for example, that someone launches activity P from the home screen,
	 * and from there goes to activity Q. The user next presses Home, and then
	 * returns to activity P. Normally, the user would see activity Q, since that is
	 * what they were last doing in P's task. However, if P set this flag to "true",
	 * all of the activities on top of it (Q in this case) were removed when the
	 * user pressed Home and the task went to the background. So the user sees only
	 ...
	 * restarted by default, but declaring a configuration with this attribute will
	 * prevent the activity from being restarted. Instead, the activity remains
	 * running and its
	 ...
	 *
	 * This attribute has four values: ["intoExisting" | "always" | "none" |
	 * "never"].
	 *
	 * For more information on these attribute values, see
	 *
	...
	/**
	 * Whether or not the activity can be instantiated by the system — "true" if it
	 * can be, and "false" if not. The default value is "true".
	 *
	 * The <application> element has its own enabled attribute that applies to all
	 * application components, including activities. The <application> and
	 * <activity> attributes must both be "true" (as they both are by default) for
	 * the system to be able to instantiate the activity. If either is "false", it
	 ...
	 * whether the task should not appear in the list of recent apps. Set "true" if
	 * the task should be excluded from the list; set "false" if it should be
	 ...
	 * applications with the same user ID. For our purposes, those components are
	 * other broadcast receivers and activities.
	 *
	 ...
	 * that can invoke the activity (see the {@link #permission()} attribute).
	 *
	 ...
	 * home screen) — "true" if it should be shut down, and "false" if not. The
	 ...
	 * operations. When the hardware-accelerated renderer is enabled, most
	 * operations in Canvas, Paint, Xfermode, ColorFilter, Shader, and Camera are
	 * accelerated. This results in smoother animations, smoother scrolling, and
	 * improved responsiveness overall, even for applications that do not explicitly
	 * make use the framework's OpenGL libraries. Because of the increased resources
	 ...
	 * representation of the activity is required on-screen. For example, icons for
	 * activities that initiate tasks are displayed in the launcher window. The icon
	 * is often accompanied by a label (see the {@link #label()} attribute).
	 *
	 * This attribute must be set as a reference to a drawable resource containing
	...
	/**
	 * A user-readable label for the activity. The label is displayed on-screen when
	 * the activity must be represented to the user. It's often displayed along with
	 * the activity icon.
	 *
	 ...
	 * types of activities. The other modes — "singleTask" and "singleInstance" —
	 * are not appropriate for most applications, since they result in an
	 ...
	 * instantiated multiple times. The instances can belong to any task and can be
	 * located anywhere in the activity stack. Typically, they're launched into the
	 * task that called
	 * {@link android.content.Context#startActivity(android.content.Intent)} (unless
	 * the Intent object contains a
	 * {@link android.content.Intent#FLAG_ACTIVITY_NEW_TASK} instruction, in which
	 * case a different task is chosen — see the {@link #taskAffinity()} attribute).
	 *
	 ...
	 * only one respect: A "singleTask" activity allows other activities to be part
	 * of its task. It's always at the root of its task, but other activities
	 * (necessarily "standard" and "singleTop" activities) can be launched into that
	 * task. A "singleInstance" activity, on the other hand, permits no other
	 ...
	 * is never called if you start another activity for a result from this
	 * activity.
	 *
	 ...
	 * match the class name given to the corresponding
	 * {@link ActivityElement#name()} attribute.
	 *
	 * The system reads this attribute to determine which activity should be started
	 * when the user presses the Up button in the action bar. The system can also
	 * use this information to synthesize a back stack of activities with
	 * TaskStackBuilder.
	 *
	 * To support API levels 4 - 16, you can also declare the parent activity with a
	 * {@link MetaDataElement} that specifies a value for
	 ...
	 * element's permission attribute applies to the activity. If neither attribute
	 * is set, the activity is not protected by a permission.
	 *
	 ...
	 * components of an application run in a default process name created for the
	 * application and you do not need to use this attribute. For our purposes,
	 ...
	 * above it in the task stack. A task whose root activity has this attribute set
	 * to "true" replaces the base Intent with that of the next activity in the
	 * task. If the next activity also has this attribute set to "true" then it will
	 ...
	 * attribute set to "false". The default value is "false".
	 *
	 ...
	 * and icons in the overview screen.
	 *
	 ...
	 * "userPortrait" | "sensor" | "fullSensor" | "nosensor" | "user" | "fullUser" |
	 * "locked"]
	 *
	 * For more information on these attribute values, see
	 *
	 ...
	 * {@link android.app.Activity#onSaveInstanceState(android.os.Bundle)} method is
	 * called. This method stores the current state of the activity in a Bundle
	...
	/**
	 * Specifies whether the activity supports Picture-in-Picture display. The
	 * system ignores this attribute if {@link #resizableActivity()} is false.
	 *
	 * This attribute was added in API level 24.
	 *
	 ...
	 * affinity conceptually belong to the same task (to the same "application" from
	 * the user's perspective). The affinity of a task is determined by the affinity
	 ...
	 * set this attribute to group them differently, and even place activities
	 * defined in different applications within the same task. To specify that the
	 ...
	 * application. The name of the default affinity for an application is the
	 * package name set by the <manifest> element.
	 *
	 * For details see
	 * {@link com.google.appinventor.buildserver.Compiler#writeAndroidManifest(java.io.File, java.util.Set)}.
	 *
	 ...
	 * {@link android.content.Context#setTheme(int)}, and may also cause "starting"
	 * animations prior to the activity being launched (to better match what the
	 * activity actually looks like).
	 *
	 * If this attribute is not set, the activity inherits the theme set for the
	 * application as a whole — from the <application> element's theme attribute. If
	 * that attribute is also not set, the default system theme is used.
	 *
	 ...
	 * make the current focus visible when part of the window is covered by the soft
	 * keyboard.
	 *
