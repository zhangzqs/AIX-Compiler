 ...
 * of an <intent-filter>. A <category> element is a string containing additional
 * information about the kind of component that should handle the intent. Any
 * number of category descriptions can be placed in an intent, but most intents
 * do not require a category.
 *
...
public @interface CategoryElement {
	 ...
	 * {@link android.content.Intent} class as CATEGORY_name constants. The name
	 * assigned here can be derived from those constants by prefixing
	 * "android.intent.category." to the name that follows CATEGORY_. For example,
	 * the string value for {@link android.content.Intent#CATEGORY_LAUNCHER} is
	 ...
	 * they are unique.
	 *
	 * The name attribute is required in any @CategoryElement annotation and hence
