...
/**
 * Adds a data specification to an <intent-filter> as a <data> subelement. The
 * specification can be just a data type {@link #mimeType()}, just a URI, or
 * both a data type and a URI. A URI is specified by separate attributes for
 * each of its parts:
 *
 * {@link #scheme()}://{@link #host()}:{@link #port()}[{@link #path()}|{@link #pathPrefix()}|{@link #pathPattern()}]
 *
 * When neither the MIME data type nor the data URI is specified, the <data>
 * element is ignored and not added to the manifest.
 *
 ...
 * https://developer.android.com/guide/topics/manifest/data-element.html </a>}.
 *
...
public @interface DataElement {
	 ...
	 * scheme attribute is also specified for the filter. If a host is not specified
	 * for the filter, the port attribute and all the path attributes are ignored.
	 *
	...
	/**
	 * A MIME media type, such as image/jpeg or audio/mpeg4-generic. The subtype can
	 * be the asterisk wildcard (*) to indicate that any subtype matches.
	 *
