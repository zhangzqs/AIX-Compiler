 ...
 *
 * Ordinary values are specified through the value attribute. However, to assign
 * a resource ID as the value, use the resource attribute instead.
 *
 * When using a MetaDataElement, the {@link #name} attribute must be specified
 * along with either the {@link #resource()} or the {@link #value()} attribute.
...
public @interface MetaDataElement {
	...
	/**
	 * A unique name for the data item. By convention, this name should follow the
	 * Java package name format, e.g. "com.example.project.activity.data". The name
	 * attribute is required in any @MetaDataElement annotation and hence has no
	 * default value.
	 *
	...
	/**
	 * A reference to a resource. The ID of the resource is the value assigned to
	 * the data item. The ID can be retrieved from the meta-data Bundle by the
	...
	/**
	 * The value assigned to the item. The data types that can be assigned as values
	 * and the {@link android.os.Bundle} methods that components use to retrieve
	 * those values are detailed below:
	 *
	 * -> A String value, using double backslashes (\\) to escape characters, such
	 * as "\\n" and "\\uxxxxx" for a Unicode character, can be accessed using
	 ...
	 *
	 * -> A Color value, in the form "#rgb", "#argb", "#rrggbb", or "#aarrggbb", can
	 * be accessed using {@link android.os.Bundle#getInt(String)}.
	 *
