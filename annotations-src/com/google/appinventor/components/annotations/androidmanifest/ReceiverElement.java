 ...
 * it can be added to AndroidManifest.xml. <receiver> elements indicate that a
 * component is a broadcast receiver. <receiver> element attributes that are not
 * set explicitly default to "" or {} and are ignored when the element is
...
public @interface ReceiverElement {
	 ...
	 *
	 * @return an array containing the <meta-data> subelements for this <receiver>
	 *         element
	 */
	 ...
	 * class name (such as, "com.example.project.ReportReceiver"). The name
	 * attribute is required in any @ReceiverElement annotation and hence has no
	 * default value.
	 *
	 ...
	 * application components, including broadcast receivers. The <application> and
	 * <receiver> attributes must both be "true" for the broadcast receiver to be
	 * enabled. If either is "false", it is disabled; it cannot be instantiated.
	 *
	 ...
	 * components of the same application or applications with the same user ID. For
	 * our purposes, those components are other broadcast receivers and activities.
	 *
	 ...
	 * the broadcast receiver is intended to receive intents broadcast by the system
	 * or other applications, so the default value is "true".
	 *
	...
	/**
	 * An icon representing the broadcast receiver. This attribute must be set as a
	 * reference to a drawable resource containing the image definition. If it is
	 ...
	 * element — is also the default icon for all the receiver's intent filters (see
	 * the {@link IntentFilterElement#icon()} attribute).
	 *
	 ...
	 * broadcast receiver's label — whether set here or by the <application> element
	 * — is also the default label for all the receiver's intent filters (see the
	 ...
	 * broadcast receiver runs in that process. If the process name begins with a
	 * lowercase character, the receiver will run in a global process of that name,
	 * provided that it has permission to do so. This allows components (broadcast
	 * receivers and activities) in different applications to share a process,
